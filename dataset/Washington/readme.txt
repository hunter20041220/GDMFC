...
===== Description =====
The archive contains 230 documents over the 5 labels (student,project,course,staff,faculty). It is made of 4 views (content,inbound,outbound,cites) on the same documents. The documents are described by 1703 words in the content view, and by the 783 links between them in the inbound, outbound and cites views.
...
washington_inbound.mtx -- 
	the matrix indicating by 0/1 values the inbound links between documents, in the Matrix Market coordinate format (sparse).

washington_outbound.mtx -- 
	the matrix indicating by 0/1 values the outbound links between documents, in the Matrix Market coordinate format (sparse). It is the transpose of washington_inbound.mtx.
...
labels.txt --
	contains the list of the different labels, in the order of the affectations found in washington_act.txt