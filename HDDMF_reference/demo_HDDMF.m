...
% Get the root directory of HDDMF
...
dataf = fullfile(datadir, [cell2mat(dataname(idata)), '.mat']);
load(dataf);
...
%%
savePath = fullfile(root_dir, 'results_HDDMF');
...
Tname = fullfile(savePath, [dataset, '.txt']);
dlmwrite(Tname, eva_spe, '-append', 'delimiter', '\t', 'newline', 'pc');
objectname = fullfile(savePath, [dataset, '.mat']);
